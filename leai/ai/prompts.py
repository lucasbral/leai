...
def build_language_directive(lang: str | None = None) -> str:
    """Builds the mandatory English instruction specifying the user's target output language."""
...
# Backward-compatible references (now deprecated in favor of canonical English prompts + directive)
TABLE_ENRICHMENT_SYSTEM_PROMPT_PT = f"{TABLE_ENRICHMENT_SYSTEM_PROMPT}\n\n{build_language_directive('pt-BR')}"
CODE_OBJECT_ENRICHMENT_SYSTEM_PROMPT_PT = f"{CODE_OBJECT_ENRICHMENT_SYSTEM_PROMPT}\n\n{build_language_directive('pt-BR')}"
ASK_SYSTEM_PROMPT_PT = f"{ASK_SYSTEM_PROMPT}\n\n{build_language_directive('pt-BR')}"


def get_table_enrichment_prompt(lang: str | None = None) -> str:
    """Returns the canonical English table enrichment prompt with the target output language directive."""
    return f"{TABLE_ENRICHMENT_SYSTEM_PROMPT}\n\n{build_language_directive(lang)}"


def get_code_enrichment_prompt(lang: str | None = None) -> str:
    """Returns the canonical English code object enrichment prompt with the target output language directive."""
    return f"{CODE_OBJECT_ENRICHMENT_SYSTEM_PROMPT}\n\n{build_language_directive(lang)}"


def get_ask_system_prompt(lang: str | None = None) -> str:
    """Returns the canonical English LEAI Copilot ask/chat prompt with the target output language directive."""
    return f"{ASK_SYSTEM_PROMPT}\n\n{build_language_directive(lang)}"
